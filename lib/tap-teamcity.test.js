...
test('tapTeamCity, not ok assertion', (t) => {
  ...
  const checkFailed = (data) =>
    t.equal(
      data,
      '\n##teamcity[testFailed name=\'second assert\' message=\'\' details=\'    operator: equal|n    expected: false|n    actual:   true|n   at: Test.<anonymous> (./tap-teamcity.js:15:5)\' type=\'comparisonFailure\' expected=\'false\' actual=\'true\']'
    )
  const checkFinish = (data) =>
    t.ok(
